...
class NotifyHelper {
  ...
  displayNotification(
      {required String title, required String body, required Task task}) async {
    print('doing test');
  ...
  tz.TZDateTime _nextInstanceOfTenAM(
    ...
    if (scheduledDate.isBefore(now)) {
      ...
      if (repeat == 'Weekly') {
        scheduledDate = scheduledDate = tz.TZDateTime(tz.local, now.year,
            now.month, formattedDate.day + 7, hour, minutes);
      }
      if (repeat == 'Monthly') {
    ...
    print('this is scheduledDate $scheduledDate');
